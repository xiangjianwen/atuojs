...
//在原算法利用高斯模糊轮廓，模板匹配找棋子
//需要准备两张图片小白点图片temp_white_circle.png
//棋子图片a3.png 放在debug目录里
//
...
function getWhileCirclePos(im) {
    ...
    //log(img_rgb);
    //小白点图片
    let temp_white_circle = Highgui.imread("/sdcard/debug/temp_white_circle.png");
    ...
    if (maxmin.maxVal > 0.9) {
        ...
        toastLog(maxmin.maxVal+'found white circle!')
    }
...
function getBottlePos(im) {
    ...
    //Imgproc.matchTemplate(img_rgb1, temp_white_circle1, res2, Imgproc.TM_CCORR_NORMED)
    Imgproc.matchTemplate(img_rgb, temp_white_circle, res2, Imgproc.TM_CCORR_NORMED)
   
    let maxmin = Core.minMaxLoc(res2)
    log(maxmin.maxVal + "棋子匹配度" + maxmin.maxLoc);
