...
var my = document.createElement("divCell");
  ...
  my.innerHTML='<div  style="position:fixed ; bottom:80px;width:1002" lign="left" id="bg"><textarea  id="name" cols="4" rows="5"></textarea> <textarea  id="card" cols="10" rows="5"></textarea><textarea  id="output" cols="10" rows="5"></textarea><input type="button" name="tijiao" id="tijiao" value="提交" /><label id ="l" style="background-color:white;">0</label  ><input type="button" name="zddt" id="bc" value="保存分数到缓存" />  <label id ="l2" style="background-color:white;">0</label  ></div>';
 var s="";//存结果
...
var s=$("#output").val()
  ...
  GM_setValue("b", cards)
  GM_setValue("c", s)
  $("#l2").text("保存长度"+" 姓名:"+names.length+"，身份证号码:"+cards.length+"，爬取数据:"+s.length)
   });
